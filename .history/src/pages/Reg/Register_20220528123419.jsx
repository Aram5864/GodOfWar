...
const [name, setname] = useState('eewrw')
const [isvalid, setisvalid] = useState('werwerwe')

    const myName = (evt) =>{
        setname(evt.target.value);

        if (name.length < 3 || name.length > 10) {
            setisvalid('inpValerror')
        }else{
            setisvalid('inpValsuccess')
        ...
        <div class= {style.wrapper}>
               ...
               <form class="p-3 mt-3">
                   <div class={`" d-flex align-items-center" ${style.form_field}`}>
                       ...
                       <input className={style.isvalid}  onChange={myName} type="text" name="userName" id="userName" placeholder="Username"/>
                   </div>
